...
public class CategoryApiController {
    ...
    public ResponseFormat<List<ScrapListDto>> scrapsByCategory(@PathVariable Integer categoryId) {
        Integer userId = 1; //임시 사용자
        List<ScrapListDto> dtos = categoryService.scrapsByCategory(userId, categoryId);
        return (dtos!=null) ?
                ResponseFormat.ok("카테고리별 스크랩 조회 완료", dtos):
                ResponseFormat.failure("요청 형식이 올바르지 않습니다.");
    }
